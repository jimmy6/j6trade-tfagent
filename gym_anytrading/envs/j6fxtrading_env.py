...
class J6FxTrading(TradingEnv):
    ...
    def _process_data(self):
        prices = self.df.loc[:, 'close'].round(1).to_numpy()
    ...
    def _calculate_reward(self, action):
        ...
        if trade:
            ...
            if self._position == Positions.Short:
                step_reward += -price_diff * 10
            elif self._position == Positions.Long:
                step_reward += price_diff * 10
            print("Trade - price_diff={0} current_price={1} last_trade_price={2} action={3}".format(price_diff, current_price, last_trade_price, action))
        return step_reward
